...
def test_multi_method_genome():
    ...
    for method in MergeMethod:
        ...
        print(
            f"  {name:18s}: {params} params, base={requires_base}, models={min_models}-{max_models}"
        )
    ...
    definition_2models = MultiMethodGenomeDefinition(
        ...
        allowed_methods=[
            ...
            "nuslerp",
            "arcee_fusion",
        ],  # All work with exactly 2 models
    ...
    model_selection_dim = (
        max_models if definition.enable_model_selection else len(definition.models)
    )
