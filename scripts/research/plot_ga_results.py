...
def plot_results(metrics: List[Dict], output_file: str = "ga_results.png"):
    ...
    if not plt or not np:
        print(
            "⚠️  Plotting requires matplotlib. Install with: pip install matplotlib numpy"
        )
        return
    ...
    width = 0.35
    ax.bar(
        x - width / 2, cache_hits, width, label="Cache Hits", alpha=0.8, color="blue"
    )
...
def print_table(metrics: List[Dict]):
    ...
    print("\n" + "=" * 120)
    print(
        f"{'Gen':<5} {'Best Score':<16} {'Mean Score':<16} {'Std Dev':<14} {'Cache':<8} {'Failed':<8} {'Crossover':<10}"
    )
    print("=" * 120)

    for m in metrics:
        print(
            f"{m['generation']:<5} {m['best_score']:<16.4e} {m['mean_score']:<16.4e} {m['std_score']:<14.4e} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract and plot GA evolution results"
    )
    parser.add_argument("log_file", help="Path to GA run log file")
    parser.add_argument(
        "--output",
        "-o",
        default="ga_results.png",
