...
try:
    ...
    result = subprocess.run(
        ["pgrep", "-f", "mergekit.scripts.evolve_ga"], capture_output=True, text=True
    )
...
try:
    ...
    if gens == 0:
        print(
            "   (First generation is still being evaluated - this can take 2-5 minutes)"
        )
...
# Check Ray dashboard
result = subprocess.run(
    ["curl", "-s", "http://127.0.0.1:8265"], capture_output=True, timeout=2
)
