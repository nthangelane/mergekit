...
- CMA-ES evolution: `docs/evolve.md`
- GA evolution: `docs/evolve_ga.md`

### Evolution (GA)

`mergekit-evolve-ga` runs a genetic algorithm to optimize merge parameters against evaluation tasks (via EleutherAI's lm-eval harness). It mirrors the flow of `mergekit-evolve` (which uses CMA-ES) and supports the same scheduling strategies, vLLM evaluation, and Weights & Biases logging.
...
mergekit-evolve-ga \
  ...
  --population-size 32 \
  --max-fevals 320 \
  --storage-path /path/to/mergekit-evolve-ga \
  ./config.yml
```

Key GA options:

- `--population-size`: number of individuals per generation (default 32)
- `--elite-fraction`: top fraction preserved each generation (default 0.125)
...
- Writes the best merge configuration to `storage_path/best_config.yaml`
- Optionally saves the final merged model to `storage_path/final_model` (default enabled)
